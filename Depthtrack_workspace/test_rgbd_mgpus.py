...
def genConfig(seq_path, set_type):
    if set_type == 'depthtrack':
        ############################################  have to refine #############################################
        RGB_img_list = sorted([seq_path + '/color/' + p for p in os.listdir(seq_path + '/color') if os.path.splitext(p)[1] == '.jpg'])
        T_img_list = sorted([seq_path + '/depth/' + p for p in os.listdir(seq_path + '/depth') if os.path.splitext(p)[1] == '.png'])

        RGB_gt = np.loadtxt(seq_path + '/groundtruth.txt', delimiter=',')
        T_gt = np.loadtxt(seq_path + '/groundtruth.txt', delimiter=',')
...
def run_sequence(seq_name, seq_home, dataset_name, yaml_name, num_gpu=1, epoch=300, debug=0, script_name='prompt'):
    ...
    save_path = f'./Depthtrack_workspace/results/vipt_deep/rgbd-unsupervised/' + seq_txt +  '/' + seq_txt + '_001' + '.txt'
    save_folder = f'./Depthtrack_workspace/results/vipt_deep/rgbd-unsupervised/' + seq_txt
    ...
    for frame_idx, (rgb_path, T_path) in enumerate(zip(RGB_img_list, T_img_list)):
        ...
        if frame_idx == 0:
            # initialization
            image = get_rgbd_frame(rgb_path, T_path, dtype='rgbcolormap', depth_clip=True)
            tracker.initialize(image, RGB_gt[0].tolist())  # xywh
        elif frame_idx > 0:
            # track
            image = get_rgbd_frame(rgb_path, T_path, dtype='rgbcolormap', depth_clip=True)
            region, confidence = tracker.track_w(image)  # xywh
...
class ViPT_RGBT(object):
    ...
    def track_w(self, img_RGB):
        '''TRACK'''
        outputs = self.tracker.track_d(img_RGB)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--yaml_name', type=str, default='deep_rgbd', help='Name of tracking method.')  # vitb_256_mae_ce_32x4_ep300 vitb_256_mae_ce_32x4_ep60_prompt_i32v21_onlylasher_rgbt
    parser.add_argument('--dataset_name', type=str, default='depthtrack', help='Name of dataset (GTOT,RGBT234,LasHeR,VTUAVST,VTUAVLT).')
