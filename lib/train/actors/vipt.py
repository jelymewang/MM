...
class ViPTActor(BaseActor):
    ...
    def __call__(self, data):
        ...
        """
        # forward pass
        if random.random() > 0.8 and data['search_images'] is not 0:
            iteration = 10
            epsilon = 0.2
            alpha = epsilon / iteration
            x_adv = data['search_images'].clone().detach()
            x_adv.requires_grad = True
            for _ in range(iteration):
                data['search_images'] = x_adv
                ...
                self.net.zero_grad()
                loss.backward()
                grad = x_adv.grad.detach().sign()  # Detach the gradient and take the sign
                x_adv = x_adv - grad * alpha
                x_adv = x_adv.clone().detach()  # Clone and detach again
                x_adv.requires_grad = True  # Set requires_grad for the next iteration
